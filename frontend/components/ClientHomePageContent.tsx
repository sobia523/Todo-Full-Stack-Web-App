...
export default function ClientHomePageContent() {
  ...
  useEffect(() => {
    if (hasMounted && !isLoading) { // Only run logic if mounted on client and not loading
      if (isAuthenticated) {
  ...
  }, [router, isAuthenticated, isLoading, hasMounted]);

  if (!hasMounted || isLoading) {
    // Render a loading state or nothing while waiting for client-side hydration and auth status
    return (
  ...
  return null; // Render nothing if not authenticated and not loading, as redirect will happen in useEffect
}